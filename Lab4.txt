...
select a.CUS_NAME,a.CUS_GENDER from customer as a where CUS_NAME like 'A%' or CUS_NAME like '%A';

select `order`.*,product.* from `order`,product where `order`.cus_id=2 group by ord_id;
...
from (select final.supp_id,supplier.supp_name,final.Average from 
...
as test2 group by supplier_pricing.supp_id) as report;
DELIMITER 
call proc1();